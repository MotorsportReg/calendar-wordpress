...
Author: MotorsportReg.com
Version: 1.3
Author URI: http://www.motorsportreg.com
*/
...
function reconstruct_url($url){
	...
	$constructed_url = $url_parts['scheme'] . '://' . $url_parts['host'] . (isset($url_parts['path'])?$url_parts['path']:'') . '?utm_source=' . urlencode($_SERVER['HTTP_HOST']) . '&amp;utm_medium=wordpress_plugin&amp;utm_campaign=motorsportreg';
...
function msr_calendar_shortcode($atts) 
{
	...
	$out = msr_calendar_view($xml, $title, $field_eventname, $field_organization, $field_venue, $field_venuecity, $field_eventtype, $field_eventdate);
	return $out;
}
...
function msr_calendar_view($xml, $title, $field_eventname, $field_organization, $field_venue, $field_venuecity, $field_eventtype, $field_eventdate)
{
	...
	if(count($events)){
		...
		foreach($events as $event)
		{
			...
			if($show){
				...
				if($flag)
					$out .='<td><a href="' . reconstruct_url($event['detailuri']) . '" class="txtlink_register" alt="Register now on MotorsportReg.com">Register</a></td>';
				else
					$out .='<td><a href="' . reconstruct_url($event['detailuri']) . '" class="txtlink_details" alt="View event details on MotorsportReg.com">Details</a></td>';
			}
	...
	$out .='</tbody></table><div class="morelink">Visit MotorsportReg.com to <a href="http://www.motorsportreg.com/calendar/">register for thousands of autocross, HPDE, race &amp; social events</a>.</div></div>';
